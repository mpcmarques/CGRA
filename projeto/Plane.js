...
Plane.prototype.addTextureCoords = function() {
	this.texCoords = [];

	var delta_t = (this.maxT - this.minT)/this.nrDivs;
	var delta_s = (this.maxS - this.minS)/this.nrDivs;

	for(i = 0; i <= this.nrDivs; i++){
		for (j = 0; j <= this.nrDivs ; j++){
			
			this.texCoords.push(j * delta_s, i * delta_t);
		}
	}
};
