...
for(var i = 0; i < this.torpedoes.length; i++){
	...
	if (torpedo.animationTime < torpedo.durationTime){
		...
		// Rotate Trapezoids
		LightingScene.prototype.rotateTrapezoid = function(rotation){
			this.trapezoidAngle += rotation;
		};
		// Move Periscópio
		LightingScene.prototype.movePeri = function(z){
			this.posPeri += z;
			console.log(this.posPeri);
		};
